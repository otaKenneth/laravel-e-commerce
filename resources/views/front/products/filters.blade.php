                ...
                @if ($filterKey == 'categories')
                    <ul class="filter-categories">
                        @foreach ($filter as $categoryKey => $category)
                            <li class=""><a
                                    href="{{ url('products/category/' . $category['url']) }}">{{ $category['category_name'] }}</a>
                            </li>
                            @if (count($category['sub_categories']) > 0)
                                <ul>
                                    @foreach ($category['sub_categories'] as $sub_category)
                                        <li class=""><a
                                                href="{{ url('products/category/' . $sub_category['url']) }}">{{ $sub_category['category_name'] }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            @endif
                ...
                @else
                    <div class="associate-wrapper">
                        @foreach ($filter as $key => $childFilters)
                            <div>
                                <input type="checkbox" class="check-box {{ $key }}"
                                    id="{{ $filterKey }}{{ $key }}" name="{{ $filterKey }}[]"
                                    value="{{ $childFilters }}">
                                <label class="label-text"
                                    for="{{ $filterKey }}{{ $key }}">{{ $childFilters }}
...
$filterAvailable = \App\Models\ProductsFilter::filterAvailable(
    $filter['id'],
    $categoryDetails['catIds'],
                ); // $categoryDetails was passed from the listing() method in the Front/ProductsController
            @endphp

            @if (count($filter['filter_values']) > 0)
                {{-- show ONLY the filters (`filter_name`) which have filter values (`filter_value`) e.g. if the `Operating System` filter doesn't have filter values like: '4GB', '6GB', ..., DON'T show it --}}
                    ...
                    <div class="associate-wrapper">
                        @foreach ($filter['filter_values'] as $value)
                            ...
                            <input type="checkbox" class="check-box {{ $filter['filter_column'] }}"
                                ...
                                value="{{ $value['filter_value'] }}"> {{-- Note!!: PLEASE NOTE THE SQUARE BRACKETS [] OF THE "name" ATTRIBUTE!! --}} {{-- echo the filter name as a 'CSS class' to be able to use it in jQuery for filtering, and also add the "name" (as an array!! PLEASE NOTE THE SQUARE BRACKETS [] !!! e.g.    'fabric' => ['cotton', 'polyester']    ) and "value" HTML attributes too --}}
                            {{-- the checked checkboxes <input> fields will be submitted as an ARRAY because we used SQUARE BRACKETS [] with the "name" HTML attribute in the checkbox <input> field in filters.blade.php e.g.    'fabric' => ['cotton', 'polyester']    , or else, AJAX is used to send the <input> values WITHOUT submitting the <form> at all --}}
                            <label class="label-text"
        ...
        <div class="facet-filter-associates">
            ...
            <input type="hidden" name="price_min" id="hidden_price_min" value="0">
            <input type="hidden" name="price_max" id="hidden_price_max" value="1000">
        </div>
        <!-- Filter-Price /- -->
