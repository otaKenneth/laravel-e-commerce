...
class V2_ProductsController extends Controller
{
    ...
    { // using the Dynamic Routes with the foreach loop
        ...
        try {
            ...
            $collection = $collection->inRandomOrder()->paginate(12);

            // Transform products for React
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    'filters' => $filters ?? [], // If this will be on another api (Genna)
            ...
            Log::error("Product Listing API Error: " . $e->getMessage());
    ...
    { // Required Parameters: https://laravel.com/docs/9.x/routing#required-parameters
        ...
        $productDetails = Product::with([
            ...
            'images',
            'vendor'
        ])->findOrFail($id); // Eager Loading (using with() method): https://laravel.com/docs/9.x/eloquent-relationships#eager-loading    // Eager Loading Multiple Relationships: https://laravel.com/docs/9.x/eloquent-relationships#eager-loading-multiple-relationships
        ...
        $avgRating = $ratingCount > 0 ? round($ratingSum / $ratingCount, 2) : 0;
            ...
            $starRatings = [];
            for ($i = 1; $i <= 5; $i++) {
                $starRatings[$i] = \App\Models\Rating::where([
                    'product_id' => $id,
                    'status' => 1,
                    'rating' => $i
                ])->count();
            ...
            $transformedProduct = [
                ...
                'vendor' => $productDetails->vendor ? [
                    ...
                    'shop_name' => $productDetails->vendor->vendorbusinessdetails->shop_name ?? null,
                ] : null,
                ...
                'section' => $productDetails->section ? [
                    ...
                    'name' => $productDetails->section->name,
                ] : null,
                'images' => $productDetails->images->toArray(),
                'attributes' => $productDetails->attributes->toArray(),
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    'category_details' => $categoryDetails,
                    'similar_products' => $similarProducts->toArray(),
                    'recently_viewed_products' => $recentlyViewedProducts->toArray(),
                    'group_products' => $groupProducts->toArray(),
                    'ratings' => [
                        'reviews' => $ratings->items(),
                        'average_rating' => $avgRating,
                        'average_star_rating' => $avgStarRating,
                        'total_reviews' => $ratingCount,
                        'star_counts' => $starRatings,
                        'pagination' => [
    ...
    // API endpoint for React - Get Product Price (for attribute changes)
    public function getProductPrice(Request $request)
    {
        try {
            ...
            return response()->json([
                ...
                'data' => $getDiscountAttributePrice
            ], 200);

        } catch (\Exception $e) {
            Log::error("Get Product Price API Error: " . $e->getMessage());
            
            return response()->json([
                'success' => false,
                'message' => 'Failed to get product price: ' . $e->getMessage(),
                'data' => null
            ], 500);
        }
    }
    ...
    public function cart()
    {
        ...
        // Transform cart items for React
            $transformedCartItems = [];
            $totalAmount = 0;
            
            foreach ($getCartItems as $item) {
                $productPrice = Product::getDiscountAttributePrice($item['product_id'], $item['color'], $item['size']);
                $itemTotal = $productPrice['final_price'] * $item['quantity'];
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    'total_items' => count($transformedCartItems),
                    'coupon_applied' => Session::has('couponCode'),
                    'coupon_code' => Session::get('couponCode'),
                    'coupon_amount' => Session::get('couponAmount', 0),
                    'grand_total' => $totalAmount - Session::get('couponAmount', 0),
                    'meta' => [
                        'title' => 'Shopping Cart - Kapiton',
                        'description' => '',
                        'keywords' => 'shopping cart, kapiton cart, kapiton e-commerce',
                    ]
                ]
            ], 200);

        } catch (\Exception $e) {
            Log::error("Cart API Error: " . $e->getMessage());
    ...
    public function cartUpdate(Request $request)
    {
        try {
            ...
            $cartDetails = \App\Models\Cart::find($data['cartid']);
            ...
            if ($data['qty'] > $availableStock->stock) {
                return response()->json([
                    ...
                    'data' => [
                        'available_stock' => $availableStock->stock,
            ...
            // Update quantity
            \App\Models\Cart::where('id', $data['cartid'])->update([
                'quantity' => $data['qty']
            ]);
            ...
            Log::error("Cart Update API Error: " . $e->getMessage());
    ...
    // API endpoint for React - Delete Cart Item
    public function cartDelete(Request $request)
    {
        try {
            ...
            $cartItem = \App\Models\Cart::find($data['cartid']);
            if (!$cartItem) {
                return response()->json([
                    'success' => false,
                    'message' => 'Cart item not found',
            ...
            \App\Models\Cart::where('id', $data['cartid'])->delete();
            ...
            return response()->json([
                ...
                'data' => [
                    'cart_items_count' => $totalCartItems,
                    'removed_item_id' => $data['cartid']
                ]
            ], 200);

        } catch (\Exception $e) {
            Log::error("Cart Delete API Error: " . $e->getMessage());
