...
class Rating extends Model
{
    ...
    { // A Rating `ratings` belongs to a User `users`, and the Foreign Key of the Relationship is the `user_id` column in `ratings` table
    ...
    { // A Rating `ratings` belongs to a Product `products`, and the Foreign Key of the Relationship is the `product_id` column in `ratings` table
    ...
    {
        $product_rating = Rating::selectRaw('ROUND(AVG(rating), 2) as average_rating')
